%% Main script for Q1 PCA part
%Percentage accuracy displayed in the end
%End to end PCA recognition in one file 
tic;
...
%% Get the projection of all the vectors on the reduced size eigen space
%L is the reduced size NxN matrix
L = transpose(X)*X;
%Get eigen decoposition of L
[vector, values] = eigs(L,k);%gives k largest eigen vector
%Get eigen vectors of matrix X
V = X*vector;
V = normalize(V);
%Get projection of the eigen vectors on this reduced size k eigen space
alpha = transpose(V)*X;
