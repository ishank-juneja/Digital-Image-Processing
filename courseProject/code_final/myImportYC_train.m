function [global_mean, class_mean, X, Xn] = myImportYC_train(total_img,training)
...
X = zeros(192*168,38*training);
Xn = zeros(192*168,38*training);%size of one image
path = '../CroppedYale/YaleB0';
class_mean = zeros(192*168,training);
for  i = 1:9
...
for  i = 10:39
  ...
  %  end
    ...
    for j = 1:training
        ...
        img =img(:);
        dummy_mean(:,1) = dummy_mean(:,1) + img(:);%for calculating the mean of class
        X(:,(i-2)*training + j) = img;
        Xn(:,(i-2)*training + j) = img;
    end
    class_mean(:,i-1) = dummy_mean/training;
    for j = 1:training
        X(:,(i-2)*training + j) =  X(:,(i-2)*training + j)  -     class_mean(:,i-1);
  %  end
    end
    end
